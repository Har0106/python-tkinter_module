...
def forward(number):
    ...
    button_backward = Button(root, text='<', width=4, command=lambda:backward(number-1), state='normal').grid(row=1, column=0, pady=5)
    ...
    if number + 1 == 4:
        button_forward = Button(root, text='>', width=4, state='disabled').grid(row=1, column=2, pady=5)
        return
...
def backward(number):
    ...
    if number+1 == 0:
        button_backward = Button(root, text='<', width=4, state='disabled').grid(row=1, column=0, pady=5)
        return
    button_backward = Button(root, text='<', width=4, command=lambda:backward(number-1)).grid(row=1, column=0, pady=5)
